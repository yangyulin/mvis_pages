---
title: CLI
description: Every mvis subcommand and its flags.
pageType: reference
wide: true
---

MVIS installs a single entry point, `mvis`, with seven subcommands.
...
Until then `mvis <subcommand> --help` is authoritative.
:::
...
```bash
mvis cam --config example_cam_run.yaml
```
...
```bash
mvis cam-imu --config cam_imu.yaml
```
...
```bash
mvis apply-calib -c recalib.yaml
mvis apply-calib --calib cam_imu.json --in raw.bag --out corrected.bag
```
...
```bash
mvis run --config any_config.yaml
```

| Flag | Description |
|---|---|
| `--config`, `-c` | Any MVIS config |
...
```bash
mvis analyze --results results/regression/dataset/run/
```
...
| `python -m mvis.analysis.regenerate_html` | Rebuild HTML reports from saved YAML + detection cache |
